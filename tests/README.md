...
|---|---|
| `levels.test.mjs` | every `levels/level_NN.json`: numbering, required fields, obstacles and balls on the 16px grid and inside the playfield -- or, for an obstacle, inside the frame around it, where only the unbreakable may be built and only the floor may be slippery -- no two obstacles in one cell, every ladder reaching a footing below and a landing above with room to stand on it, player starts that are in bounds and not inside an obstacle or a ball, guaranteed drops on a single breakable block, and every type/shape/powerup/background/weapon it names actually existing |
| `assets.test.mjs` | the files the game asks for at boot are there, `elements/index.json` matches the element files on disk, the player's spritesheet holds every frame its animations name, and every sound played by name -- from code, from element data, from `regions.json` -- is defined in `audio.json` (and every `.ogg` is reachable), and a slippery obstacle's `grip` is inside 0..1 |
| `animations.test.mjs` | `js/animations.js`, which both the game and the admin tool's sprite studio read: unique keys, a texture file behind every animation, real frame indices and a rate that is not zero, the player's states matching `PLAYER_ANIM_FRAMES` frame for frame and staying inside the sheet, every effect sheet playing all of its frames, and every ball having a pop while only hex balls spin |
| `precache.test.mjs` | that the admin tool's PHP (`admin/includes/precache.php`) and `tools/build_precache.mjs` write the same offline manifest -- same hashes, same version string, same bytes -- and that a save on a host with no service worker files still succeeds. Skips itself where there is no `php` |
| `obstacles.test.mjs` | `js/obstaclePieces.js`: the piece sizes the editor offers, and the merge that turns painted cells back into obstacles on save -- a column becoming one 16x64, materials never merging into each other, a cell holding a drop staying a one-block crate, and (the one that matters) the merged obstacles covering exactly the painted cells, each of them once, whatever order they arrive in |
...
| file | what it does |
|---|---|
| `smoke.test.mjs` | boots the game; checks each weapon leaves its own mark where it stops on what it cannot break, and that rattling the trigger shakes a hanging grapple down by the 1.5s it is worth and no more; loads and runs **every** level it ships; walks, shoots and pops with real key events; loses a life and watches the ghost leave and the level restart; clears a level and lands on the next; erases progress and checks what survived; starts a blank level in the editor; checks a save with no admin tool says it is local, that every ball brush is labelled distinctly in a panel that fits, and that a guaranteed drop lands only on what can be broken open and is visible there; pauses, resumes and quits |
...
at import time -- `constants.js`, `config.js`, `assets.js`, `animations.js`,
`regions.js`, `keys.js`, `storage.js` and `elements.js` are all safe. For one that does, stub the
