...
|---|---|
| `levels.test.mjs` | every `levels/level_NN.json`: numbering, required fields, obstacles and balls on the 16px grid and inside the playfield, no two obstacles in one cell, ladders and player starts in bounds, and every type/shape/powerup/background/weapon it names actually existing |
...
| `rules.test.mjs` | the pure decisions: the playfield geometry that the placement grid and the step-up depend on, the display sizes, the transition registry's shape, one default key per action with no key doing two jobs, and key labels the menu font can actually draw |
