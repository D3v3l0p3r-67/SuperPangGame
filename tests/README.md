...
|---|---|
| `levels.test.mjs` | every `levels/level_NN.json`: numbering, required fields, obstacles and balls on the 16px grid and inside the playfield (the floor strip excepted, where a level may lay one row of something unbreakable -- an icy floor), no two obstacles in one cell, every ladder reaching a footing below and a landing above with room to stand on it, player starts that are in bounds and not inside an obstacle or a ball, guaranteed drops on a single breakable block, and every type/shape/powerup/background/weapon it names actually existing |
