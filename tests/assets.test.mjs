...
import { elements, exists, readJSON, listFiles, readText, pngSize } from './helpers.mjs';
import {
...
test('the player sheet holds every frame the animations ask for', () => {
  // One 32x64 cell per frame, stacked (see assets.js). A sheet a frame
  // short does not fail to load -- Phaser just hands out an empty frame,
  // and the player vanishes in whichever state used it.
  const needed = Math.max(...Object.values(PLAYER_ANIM_FRAMES).flat()) + 1;
  const size = pngSize(PLAYER_TEXTURE_PATH);
